...
#SOMETHING NEW TO MAKE PROGRAM RUNTIME SHORT
for x in range(len(programs_link_1)):
  link2 = ''
  if(year1 == curr_year):
    try:
      link1 = programs_link_1[x]  
    except IndexError:
      print("Index Out of range. In A-Z programs, programs list are not equal")
      continue 
    #a, b = link1.split('/ ' + year1 + '/', 1)

    # New way to split, splits the link by '/' and grabs the end of the list to check it with the array of links to find the proper link
    b = link1.split('/')
    b = list(filter(None, b))
    b = b[len(b) - 1]

    for x in range(len(programs_link_2)): 
      if b in programs_link_2[x]:
        link2 = programs_link_2[x]   

    if not link2:
      print('\n' + "No Match found for: " + link1)
      f2 = 'URL_ERROR.xlsx'
      if not (os.path.isfile(f2)):
        Workbook().save(f2)
      wb = openpyxl.load_workbook(filename = f2)
      ws = wb['Sheet']
      row_c = len(ws['A'])
      print(row_c)
      if((ws.cell(column = 1, row = row_c)).value is None):
        ws.cell(column = 1, row = row_c, value = link1)
      else:
        ws.cell(column = 1, row = row_c + 1, value = link1)
      wb.save(filename = f2)
      continue
     
  else:
    try:
      link1 = programs_link_2[x]
    except IndexError:
      print("Index Out of range. In A-Z programs, programs list are not equal")
      continue
    #a, b = link1.split('/' + year2 + '/', 1)
    ...
    for x in range(len(programs_link_1)):
      if b in programs_link_1[x]:
        link2 = programs_link_1[x]
  ...
  if response.status_code != 200:
    print('\n' + 'This link is bad:' + link1)
    continue
  
  response = requests.get(link2)
  if response.status_code != 200:
    print('\n' + 'This link is bad:' + link2)
    continue
  ...
  if(TitleEqual):
    ...
    if(containerIdentifier(driver2) != 'none'):
      ...
      #driver2.execute_script("arguments[0].click();", element2)
